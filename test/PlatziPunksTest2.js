...
describe("PlatziPunksTest2", function () {
    let platzi_punks;
    ...
    describe("tokenURI", async function () {
        ...
        it("Should name token with correct number based on tokenId", async function () {
            ...
            first = await platzi_punks.tokenURI(0);
            expect(await platzi_punks.tokenURI(0)).to.includes(Buffer.from('{"name": "PlatziPunk #0').toString('base64').slice(0, -1));
            expect(await platzi_punks.tokenURI(1)).to.includes(Buffer.from('{"name": "PlatziPunk #1').toString('base64').slice(0, -1));
        });
    })
